...
function makeApp(messages: TuiMessage[] = []): { lastFrame: () => string | undefined } {
  return render(
    React.createElement(App, { messages, status: STATUS, logo: LOGO, selectionStore: createSelectionStore(), spinnerStore: createSpinnerStore(), completionStore: createCompletionStore(), input: '', cursor: 0 }),
  );
}
...
describe('App 顶层布局（flexbox footer 紧贴 + LOGO 固定区）', () => {
  ...
  it('StatusBar 进度条随 contextPct 变化', () => {
    ...
    const { lastFrame } = render(
      React.createElement(App, { messages: [], status: status50, logo: LOGO, selectionStore: createSelectionStore(), spinnerStore: createSpinnerStore(), completionStore: createCompletionStore(), input: '', cursor: 0 }),
    );
    const frame = lastFrame() ?? '';
