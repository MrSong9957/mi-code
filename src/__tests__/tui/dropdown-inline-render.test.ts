...
// 验证：filter → candidates → renderFooter → stdout 含正确候选 + 选中反白。
...
describe('completionStore → InlineRenderer.renderFooter 数据流', () => {
  ...
  it('filter("") → renderFooter 输出含全部候选且 index=0 反白', async () => {
    ...
    // 第一个候选（index=0）必须反白
    const firstCandidate = s.candidates[0]!;
    expect(output).toContain(`\x1b[7m ▸ /${firstCandidate}`);
    // 全部候选都应出现（≤8 条可见）
    const visibleCount = Math.min(s.candidates.length, 8);
    for (let i = 0; i < visibleCount; i++) {
      expect(output).toContain(`/${s.candidates[i]}`);
    }
  });
  ...
  it('cycle → renderFooter 反白第二个候选', async () => {
    const { InlineRenderer } = await import('../../tui/inline/InlineRenderer.js');
    const store = createCompletionStore();
    store.getState().filter('');
    ...
    // 第二个候选反白，第一个不反白
    expect(output).toContain(`\x1b[7m ▸ /${s.candidates[1]}`);
    expect(output).not.toContain(`\x1b[7m ▸ /${s.candidates[0]}`);
  });
  ...
  it('selectedIndex=9（超出首屏）→ 第 9 条必须可见且反白，第 0 条滚出窗口', async () => {
    const { InlineRenderer } = await import('../../tui/inline/InlineRenderer.js');
    const store = createCompletionStore();
    ...
    // 第 9 条必须反白可见（居中）
    expect(output).toContain(`\x1b[7m ▸ /${s.candidates[9]}`);
    // 第 0 条已滚出窗口（窗口起始约 5，显示 5~12）
    expect(output).not.toContain(`/${s.candidates[0]}`);
  });

  it('selectedIndex=17（末项）→ 第 17 条必须可见且反白', async () => {
    const { InlineRenderer } = await import('../../tui/inline/InlineRenderer.js');
    const store = createCompletionStore();
    store.getState().filter('');
    const s = store.getState();
    ...
    // 末项必须反白可见
    expect(output).toContain(`\x1b[7m ▸ /${s.candidates[lastIndex]}`);
    // 第 0 条已滚出窗口
    expect(output).not.toContain(`/${s.candidates[0]}`);
  });

  it('少量候选（3 条）不滚动：全部可见，selectedIndex=2 反白末项', async () => {
    const { InlineRenderer } = await import('../../tui/inline/InlineRenderer.js');
    const three = ['config', 'compact', 'context'];
    const renderer = new InlineRenderer(process.stdout);
    ...
    // 第 2 条（末项）反白，前两条不反白
    expect(output).toContain(`\x1b[7m ▸ /${three[2]}`);
    expect(output).not.toContain(`\x1b[7m ▸ /${three[0]}`);
    expect(output).not.toContain(`\x1b[7m ▸ /${three[1]}`);
  });

  // ── 加固：反向滚动（↑ 从 index=0 循环到末项）──
  // cyclePrev 在 index=0 时跳到末项，这是真实用户路径。
  // 正向（↓）和反向（↑）到末项的 startIndex 计算应一致。
  it('selectedIndex=末项（↑ 从 0 循环回来）→ 末项可见反白，首项滚出窗口', async () => {
    const { InlineRenderer } = await import('../../tui/inline/InlineRenderer.js');
    const store = createCompletionStore();
    store.getState().filter('');
  ...
  it('selectedIndex=999（越界）→ 不崩溃、不反白任何候选', async () => {
    const { InlineRenderer } = await import('../../tui/inline/InlineRenderer.js');
    const store = createCompletionStore();
    store.getState().filter('');
    const s = store.getState();
  ...
  it('候选恰好 8 条 → 不滚动，selectedIndex=7 时末项可见反白', async () => {
    ...
    // 末项（index=7）反白，首项不反白
    expect(output).toContain(`\x1b[7m ▸ /${eight[7]}`);
    expect(output).not.toContain(`\x1b[7m ▸ /${eight[0]}`);
  });

  // ── 加固：空 suggestions 数组（InlineApp 关闭下拉时传 []）──
  it('空 suggestions → 无候选行、无反白、不崩溃', async () => {
    const { InlineRenderer } = await import('../../tui/inline/InlineRenderer.js');
    const renderer = new InlineRenderer(process.stdout);
