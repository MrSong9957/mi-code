...
// - createSpinnerStore 接收 translator，start 后 verb 来自该语言内置词库。
...
import { sampleVerb } from '../../tui/state/spinner-verbs.js';
...
describe('spinner 本地化', () => {
  ...
  describe('sampleVerb', () => {
    it('zh：返回值在内置中文动词数组内', () => {
      ...
      for (let i = 0; i < 20; i++) {
        const v = sampleVerb(undefined, zhTranslator);
        expect(pool).toContain(v);
      }
    });

    it('en：返回值在内置英文动词数组内', () => {
      const pool = enUS.spinner.builtinVerbs as readonly string[];
      for (let i = 0; i < 20; i++) {
        const v = sampleVerb(undefined, enTranslator);
        expect(pool).toContain(v);
      }
    });

    it('配置的 custom verb 原样返回（replace 模式，不翻译）', () => {
      expect(sampleVerb({ verbs: ['Customizing'], mode: 'replace' }, zhTranslator))
        .toBe('Customizing');
    });
  });

  describe('createSpinnerStore', () => {
    it('zh：start 后 verb 来自中文内置词库', () => {
      const store = createSpinnerStore(undefined, undefined, zhTranslator);
      store.getState().start('responding');
      const pool = zhCN.spinner.builtinVerbs as readonly string[];
      expect(pool).toContain(store.getState().verb);
    });

    it('en：start 后 verb 来自英文内置词库', () => {
      const store = createSpinnerStore(undefined, undefined, enTranslator);
      store.getState().start('responding');
      const pool = enUS.spinner.builtinVerbs as readonly string[];
      expect(pool).toContain(store.getState().verb);
    });
