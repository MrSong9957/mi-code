...
describe('highlightCode', () => {
  describe('JavaScript / TypeScript', () => {
    it('关键字上色（cyan）', () => {
      ...
      expect(kw.every(c => c.style.fg === 'cyan')).toBe(true);
    });
  ...
  describe('其它语言', () => {
    it('bash：# 注释 + 命令关键字', () => {
      ...
      expect(after.some(c => c.style.dim || c.style.fg === 'gray')).toBe(true);
    });

    it('python：def/return 关键字 + # 注释', () => {
      const cells = highlightCode('def f():\n  return 1 # x', 'py');
      const defCells = cells.slice(0, 3);
      expect(defCells.every(c => c.style.fg === 'cyan')).toBe(true);
    ...
    it('json：键字符串 + 数字', () => {
      ...
      expect(one?.style.fg).toBe('yellow');
    });
  });
  ...
  describe('未知语言降级', () => {
    it('未知语言不报错，返回原文（单色 dim cyan）', () => {
      ...
      // 降级为单色（全部 dim 或 cyan，至少不崩）
      expect(cells.every(c => c.style.dim || c.style.fg === 'cyan' || Object.keys(c.style).length === 0)).toBe(true);
    });
    it('空 lang 当未知处理', () => {
  ...
  describe('多行代码', () => {
    it('多行代码：每行独立高亮，换行保留', () => {
      ...
      expect(firstConst.every(c => c.style.fg === 'cyan')).toBe(true);
      expect(secondConst.every(c => c.style.fg === 'cyan')).toBe(true);
    });
