...
//
// 修复：断言光标的**绝对位置**（应在输入框行=块顶+2），而非仅比较相对漂移。
// 块顶=起始行（logo3+hook1=行4），输入框=块顶+2=行6（0-based）。
// （footer lines 顶部有 1 行间隔位 + border，输入框在第 2 行）
...
describe('覆写模式折行光标回归', () => {
  ...
  it('超宽含ANSI statusText：追加后光标在输入框行（绝对位置=块顶+1）', () => {
    ...
    // 绝对位置断言：光标应在输入框行 = 块顶(4) + 2 = 行6
    // （footer lines 顶部有 1 行间隔位 + border，输入框在第 2 行）
    expect(sim.row).toBe(6);
  });
