...
// - footerHeight = lines.length（border + 折行后输入行 + border + 折行后 status）。
 ...
 *
 * 行序：border / wrapLine(prefix+每输入行) / border / wrapLine(status)
 */
 ...
 * writtenLineCount = newHeight；cursorPhysLine0 = 1(border) + layout.row；
...
describe('物理行折算回归（Bug 4）', () => {
  ...
  it('短输入不折行：物理行=逻辑行，footerHeight=4（border+1行+border+status）', () => {
    ...
    // footerHeight = 1(border) + 1 + 1(border) + 1 = 4
  ...
  it('多行超宽：每行独立 wordWrap 后求和', () => {
    ...
    // footerHeight = 1(border) + 行0折行数 + 行1折行数 + 1(border) + 1(status)
  ...
  it('光标定位契约：单行 wordWrap 后光标 cursorUp 数值正确（光标在末尾）', () => {
    expect.hasAssertions();
    // 200a wordWrap 成 4 行（块内 0-based 物理：0=border，1..4=输入行，5=border，6=status）。
    // 光标在 200 字符末尾，layoutInputCursor 算得 row=3 → 块内 = 1+3 = 4。
    // footerHeight=7，append 模式 writtenLineCount=7，上移 = 7-4 = 3。
    const text = 'a'.repeat(200);
    renderer.renderFooter(text, 200, 'status', 80);
  ...
  it('光标定位契约：光标在行首（cursorPos=0）cursorUp 正确（wordWrap 后）', () => {
    expect.hasAssertions();
    // 光标在行首：layoutInputCursor 算得 row=0 → 块内 = 1（border 下方第1行）。
    // footerHeight=7，append 模式 writtenLineCount=7，上移 = 7-1 = 6。
    const text = 'a'.repeat(200);
