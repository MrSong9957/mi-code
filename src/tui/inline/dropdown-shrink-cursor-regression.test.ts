...
describe('下拉菜单高度收缩时光标不漂移', () => {
  ...
  it('8 候选 → 0 候选：光标必须回到输入框行（行1），不漂移', () => {
    const tracker = new CursorTracker();
    tracker.reset(0);

    // 首帧：8 候选，追加模式，footerHeight 0 → 12
  ...
  it('0 候选 → 8 候选（扩张 4→12）：光标回到输入框行，双向保护', () => {
    const tracker = new CursorTracker();
    tracker.reset(0);

    // 首帧：无下拉，追加模式，footerHeight 0 → 4
  ...
  it('候选数 18→3→18 剧烈跳变：光标始终回到输入框行（不漂移）', () => {
    ...
    // footerHeight 在 6↔20 间跳变，考验覆写状态机在非零中间值的稳定性。
    const tracker = new CursorTracker();
    tracker.reset(0);
    ...
    // 首帧：18 候选（footerHeight 0 → 22）
    renderer.renderFooter('/', 1, 'S', 80, eighteen, 0);
    for (const s of mock.written) tracker.apply(s);
    expect(tracker.row, '18候选后').toBe(1);
    mock.written.length = 0;

    // 第二帧：缩到 3 候选（footerHeight 22 → 7）
    renderer.renderFooter('/c', 2, 'S', 80, three, 0);
    for (const s of mock.written) tracker.apply(s);
    expect(tracker.row, '缩到3候选后').toBe(1);
    mock.written.length = 0;

    // 第三帧：扩回 18 候选（footerHeight 7 → 22）
    renderer.renderFooter('/', 1, 'S', 80, eighteen, 5);
    for (const s of mock.written) tracker.apply(s);
    expect(tracker.row, '扩回18候选后').toBe(1);
    mock.written.length = 0;

    // 第四帧：再次缩到 3 候选
    renderer.renderFooter('/c', 2, 'S', 80, three, 2);
    for (const s of mock.written) tracker.apply(s);
    expect(tracker.row, '再次缩到3候选后').toBe(1);
  });
});
