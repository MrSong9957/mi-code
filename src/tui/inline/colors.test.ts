...
describe('色板模块 - SGR 常量', () => {
  it('RESET 是 SGR 0（所有属性归零）', () => {
    expect(RESET).toBe('\x1b[0m');
  });

  it('cyanBright 是 SGR 96（LOGO + mode 主色）', () => {
    expect(cyanBright).toBe('\x1b[96m');
  });

  it('magentaBright 是 SGR 95（model）', () => {
    expect(magentaBright).toBe('\x1b[95m');
  });

  it('yellowBright 是 SGR 93（dir）', () => {
    expect(yellowBright).toBe('\x1b[93m');
  });

  it('greenBright 是 SGR 92（branch）', () => {
    expect(greenBright).toBe('\x1b[92m');
  });

  it('blueBright 是 SGR 94（context）', () => {
...
describe('colorizeLogo - LOGO 全行单色着色', () => {
  it('整行用 cyanBright 包裹', () => {
    const line = ' ▐▛███▜▌   MiCode v1.0.0';
    expect(colorizeLogo(line)).toBe('\x1b[96m ▐▛███▜▌   MiCode v1.0.0\x1b[0m');
  });
  ...
  it('三行 LOGO 各自独立着色（每行都有独立的色码和 reset）', () => {
    ...
    const colored = lines.map(colorizeLogo);
    // 每行都以 cyanBright 开头、reset 结尾
    for (const c of colored) {
      expect(c.startsWith('\x1b[96m')).toBe(true);
      expect(c.endsWith('\x1b[0m')).toBe(true);
    }
  });
});

describe('colorizeStatus - 状态栏 5 字段分色着色', () => {
  ...
  it('5 个字段分别用不同亮色，用 │ 分隔（分隔符不着色）', () => {
    const result = colorizeStatus(fields);
    // 分隔符 │ 不被颜色包裹（前后是 reset 和色码）
    expect(result).toContain('\x1b[0m │ \x1b[95m'); // mode 和 model 之间
    // 5 个字段各自的色码
    expect(result).toContain('\x1b[96mauto\x1b[0m');       // mode: cyanBright
    expect(result).toContain('\x1b[95mgpt-4o\x1b[0m');     // model: magentaBright
    expect(result).toContain('\x1b[93mmi-code\x1b[0m');    // dir: yellowBright
    expect(result).toContain('\x1b[92mmain\x1b[0m');       // branch: greenBright
    expect(result).toContain('\x1b[94m████░░░░░░ 40%\x1b[0m'); // context: blueBright
  });
  ...
  it('进度条和百分比作为整体用 blueBright 包裹', () => {
    const result = colorizeStatus({ ...fields, context: '██ 50%' });
    // context 整体被 blueBright 包裹，进度条和百分比不分开
    expect(result).toContain('\x1b[94m██ 50%\x1b[0m');
  });
  ...
  it('不同字段值着色正确（值变化不影响色码分配）', () => {
    ...
    expect(result).toContain('\x1b[96mplan\x1b[0m');
    expect(result).toContain('\x1b[95mclaude-3\x1b[0m');
    expect(result).toContain('\x1b[93mother-project\x1b[0m');
    expect(result).toContain('\x1b[92mfeat/x\x1b[0m');
    expect(result).toContain('\x1b[94m░░░░░░░░░░ 0%\x1b[0m');
  });
});
