...
//   footerHeight = 1(border) + 1(input) + 1(border) + statusWrapLines。
//
...
/** 算期望 footerHeight = 1(border) + inputWrapLines + 1(border) + statusWrapLines。
...
function expectedFooterHeight(statusText: string, usableWidth: number): number {
  ...
  return 1 + inputPhys + 1 + statusPhys;
}
...
describe('超宽 statusText 截断回归', () => {
  ...
  it('短 statusText（不超宽）：footerHeight=4（不受影响）', () => {
    expect.hasAssertions();
  ...
  it('超宽含ANSI statusText：光标 cursorUp 值正确（弥补 upFromBottom 盲区）', () => {
    ...
    // 空输入：光标在输入框行（块内物理行1），newHeight=expectedFooterHeight，
    // cursorPhysLine0 = 1（空输入 wrapLine=1 行，layout row=0），
    // upFromBottom = newHeight - 1。
    const statusText = '\x1b[36m' + 'X'.repeat(120) + '\x1b[0m';
    expect(stringWidth(statusText)).toBeGreaterThan(cols);
    ...
    const cursorPhysLine0 = 1; // 空输入 layout row=0 → 跳过顶部 border 后 = 1
