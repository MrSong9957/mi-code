...
### 🎨 自研 TUI 渲染引擎
- **差量渲染**：基于 `DoubleBuffer` 的双缓冲 diff 引擎，只重绘变化区域
...
### 🔐 权限系统
- 三级模式：`build`（交互确认）、`plan`（只读）、`auto`（受控自动：危险操作仍拦截，`run_bash` 需经安全分类审核）
- 工具级权限控制
- 模式即时切换

### 🌐 多语言
- 支持 `zh-CN` / `en-US`（默认 `zh-CN`）
- `/language [lang]`：查看或切换 UI 语言（持久化；`中文` / `英文` 亦可作为别名）
- `--language <zh-CN|en-US>`：仅本次启动临时覆盖（不持久化）
- Agent 回复语言随 UI 语言，但仅为软偏好：用户明确要求或项目规则另行指定时以其为准
