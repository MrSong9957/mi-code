...
每列最小内容宽度为下列值的最大值：该列标题的显示宽度、该列标题及所有
数据单元格中任一可见字符的显示宽度，以及 `1`。即：

```ts
minimumWidths[col] = Math.max(
  headerDisplayWidth,
  maxSingleCharWidthInColumn(col),
  1,
);
```

这保证宽度为 2 的 CJK 字符不会被放入宽度为 1 的列。加上左右 padding 后，
每列至少能完整容纳标题，并能容纳该列的任一可见字符。
